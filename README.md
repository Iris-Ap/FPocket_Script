...
**tamoxifen** (`OHT`) bound in its pocket. We remove the drug, run fpocket on the
bare protein, and check that fpocket independently rediscovers the real drug site.

## What `pipeline.sh` does

1. **Download** `3ERT.pdb` from the RCSB PDB.
...
5. **Prep** `pocket1.pdb` for visualization.

Result: Pocket 1 scores **0.954 druggability** and overlaps the drug site
(**1.23 Å** center-to-center, **100%** of drug atoms inside) → fpocket validated.
